...
class spectrograph_gui(QWidget):
    def __init__(self, hardware=None, model=None, fname=None, data_type=None, sim_type=None, parent=None, step=True, csv_name='eeg_log_file', arduino=False, arduino_port=0):
        ...
        if data_type == 'Live stream':
            ...
            raise Exception(
                'Unknown data type: {} Try "Live stream", "Simulate", or "File"'.format(data_type))
        ...
        self.main_graph_raw_trace.clicked.connect(
            self.graph_main_graph_raw_trace)
        ...
        self.main_graph_spectrogram.clicked.connect(
            self.graph_main_graph_spectrogram)
        ...
        self.window_resize = plus_minus_button(self.layout, 80, 55, 5, 10, 6, "Window Size", lambda t: (
            self.expand_window_sizes(t), self.get_window_time_range()))
        self.step_resize = plus_minus_button(
            self.layout, 90, 55, 5, 10, 6, "Step Size", lambda t: (True, self.data_points_to_range(t)))
    ...
    def init_hardware_type(self):
        ...
        print('init hardware is running with hardware',
              self.hardware, 'model', self.model)
    ...
    def activate_arduino(self):
        if self.arduino_process is None:
            self.arduino_process = Process(target=arduino_run, args=(
                int(self.arduino_port),), name='arduino process')
    ...
    def start_data_stream(self):
        ...
        if self.data_type == SIMULATE:
            ...
            if self.sim_type == 'Awake':
                self.sending_data = Process(target=sim_awake_eeg, args=(
                    self.srate, self.channels,), name='sim data stream process', daemon=True)
            elif self.sim_type == 'Asleep':
                self.sending_data = Process(target=sim_asleep_eeg, args=(
                    self.srate, self.channels,), name='sim data stream process', daemon=True)
            else:
                self.sending_data = Process(target=send_eeg, args=(
                    self.srate, self.channels, True,), name='sim data stream process', daemon=True)
            self.sending_data.start()
            self.receiving_data = Process(target=receive_eeg, args=(
                gq, False, self.channels,), name='receiving data process', daemon=True)
            self.receiving_data.start()
        elif self.data_type == LIVESTREAM:
            if self.hardware == 'Muse':
                self.sending_data = Process(target=send_muse, args=(
                    self.srate, self.channels,), name='hardware data stream process', daemon=True)
                self.sending_data.start()
                self.receiving_data = Process(target=receive_eeg, args=(
                    gq, False, self.channels,), name='receiving data process', daemon=True)
                self.receiving_data.start()
                ...
                if self.model == 'Ganglion':
                    ...
                    self.sending_data = Process(target=send_openbci, args=(
                        self.channels, self.com_port), name='hardware data stream process', daemon=True)
                    self.sending_data.start()
                self.receiving_data = Process(target=receive_eeg, args=(
                    gq, False, self.channels,), name='receiving data process', daemon=True)
                self.receiving_data.start()
            ...
            if self.step == False:
                ...
                self.reading_file = Process(target=read_file, args=(
                    self.fname, self.hardware, self.model, gq, self.srate, self.channels,), name='file reading process', daemon=True)
    ...
    def closeEvent(self, event):
        ...
        if self.data_type != FILE:
            ...
            with open(self.csv_name, mode='a', newline='') as file:
                ...
                fwriter.writerow(
                    ["\"eeg"+str(i)+"\"" for i in range(self.channels)])
    ...
    def read_in_file(self):
        ...
        with open(self.fname, mode='r') as file:
            ...
            for row in csv_reader:
                ...
                sample = row[3:3+self.channels]
                for i in range(len(sample)):
                    sample[i] = float(sample[i])
                # print('fr read {}'.format(sample))
                self.cur.execute(
                    "INSERT INTO data VALUES (" + ','.join('?' for _ in range(self.channels)) + ")", sample)
                line_count += 1
    ...
    def update_eeg(self):
        ...
        if not gq.empty():
            ...
            self.cur.executemany(
                "INSERT INTO data VALUES (" + ','.join('?' for _ in range(self.channels)) + ")", samples)
    ...
    def graph_full_length(self):
        ...
        if self.lock_to_end_clicked:
            ...
            # 2x since the content represents dist of window panel from middle
            self.window_left = self.window_right - self.window_resize.get_content() * 2

        ax.vlines([self.window_left, self.window_right],
                  0, 1, color='black', linewidth=3)
        ax.vlines([i for i in range(self.srate, self.csv_length, self.srate)],
                  0, 1, color='black', linewidth=1, linestyles='dashed')
    ...
    def graph_hypnogram(self):
        ...
        self.hypnogram.axes.step(
            [i for i in range(len(means))], means > np.mean(means))
        self.hypnogram.axes.set(
            title="Hypnogram", ylim=(-0.1, 1.1), yticks=[0, 1], yticklabels=["Other", "SWS"])
        self.hypnogram.axes.set_ylabel(
    ...
    def get_main_graph_data(self):
        ...
        if self.lock_to_end_clicked:
            self.cur.execute("SELECT * FROM data ORDER BY rowid DESC LIMIT ?;",
                             (int(self.window_right - self.window_left),))
            self.shown_data = np.array([data for data in self.cur])[::-1]
        elif self.data_changed:
            self.cur.execute("SELECT * FROM data WHERE rowid BETWEEN ? AND ?",
    ...
    def main_graph_boiler(self):
        ...
        self.main_graph.axes.set_xticklabels(
            np.round(tick_labels, decimals=3))  # labels along the bottom
    ...
    def graph_main_graph_spectrogram(self):
        ...
        # N = 256
        if len(self.shown_data) and self.shown_data.shape[1] < 256 * 5:
        ...
        self.main_graph.axes.imshow(
            self.spectrogram, origin='lower', aspect='auto')

        self.main_graph.axes.set_ylabel('Frequency (Hz)')
        self.main_graph.axes.set_yticks(
            [-0.5, 15.5, 31.5, 47.5, 63.5, 79.5, 95.5, 111.5, 127.5])
        self.main_graph.axes.set_yticklabels([None, str(u"\u03B4"), None, str(
            u"\u03B8"), None, str(u"\u03B1"), None, str(u"\u03B2"), None])
        self.main_graph_boiler()
        if self.main_graph_drawn:
            self.update_graph(self.main_graph)
        self.main_graph_drawn = True
    ...
    def expand_window_sizes(self, size_diff):
        ...
        if not (size_diff > 0 and midpoint - size_diff >= 0) or not (midpoint + size_diff < self.csv_length):
            print(
                "Cannot expand window, doing so would push window beyond graph boundaries")
            return False
        else:
...
class plus_minus_button(QWidget):
    ...
    def increment_content(self):
        if self.size_i < len(self.sizes) - 1:
            ...
            return
        adjust_success, (left, right) = self.graph_adjustment(
            int(self.sizes[self.size_i]))
        if adjust_success:
...
if __name__ == "__main__":
    ...
    ui = spectrograph_gui(hardware='Muse', model='Muse S',
                          data_type='Live stream', parent=Form)
